...
def daily_report(client, user_id, hours = 0)
  report = UserReport.new(client, user_id) do |r|
    ...
    r.h3 {r.center "Participated from: #{participation_start} to: #{participation_end}", :style=> "color:#909090 ";}
    ...
    if(not r.get_edge_type() == "browser_only") #don't do any of this because we have no windows data. THIS BLOCK DOES NOT PREVENT FIREWALL SETTINGS
     ...
     r.br
     if (not r.get_edge_type() == "browser_only" and not r.get_edge_type() == "both_no_usage_fig")
    ...
    if(not r.get_edge_type == "browser_only")
      ...
      puts "skipping windows updates if only has browser data"
    end
    ...
    r.br
    res = r.center {r.query("select essid as Name,
    ...
    if(not r.get_edge_type() == "windows_only")
      ...
      r.br
      stmt = r.center {r.query ("SELECT root_domain as 'Website',
      ...
      if(not r.get_edge_type() == "windows_only")
        ...
        puts "skipping site visit graphs (days & hours) because no browser data"
      end
    ...
    r.br
    if(not r.get_edge_type() == "windows_only" and not r.get_edge_type() == "both_no_password_fig" and not r.get_edge_type() == "both_delete")
